...
async function generateAnswer(context: string, query: string) {
    const messages = [
        {
            role: "system",
            content: "You are a helpful e-commerce assistant. Use the product info provided to answer the user's question. please reply in the langauge that the user uses, if there is no product info provided answer the question normally and say  there is no similar product for your question and say they can visit asoss.com .",
        },
        {
            role: "user",
            content: `${context}\n\nQ: ${query}`,
